...
contract DynamicSvgNft is ERC721 {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "data:applicaiton/json;base64,";
    }

    function tokenURI(
        uint256 tokenId
    ) public view override returns (string memory) {
        require(_exists(tokenId), "Uri request for nonexistent token.");
        // string memory imageURI = "YOO!";

        // prefix
        // data:applicaiton/json;base64,
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                '"attributes": [{"traits_type": "coolness", "value": 100}] , "image":"',
                                imageURI,
                                '"}'
                            )
                        )
